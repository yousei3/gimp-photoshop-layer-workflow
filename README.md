...
This suite is specifically developed and tested for the **GIMP 3.0 series (including 3.2)** on **Linux Mint** and **Windows 10**.
...
* **Merge Selected Layers**: Merges selected layers into a single new layer. (Note: This is a destructive operation that replaces the original layers with the merged result, similar to the Photoshop workflow.)
...
4. **[Linux Users Only]** Ensure the `.py` files have execution permissions:
```bash
chmod +x *.py
...
5. Restart GIMP. You will find the new commands under the **Layer** menu and the **Layers dock right-click menu**.
...
My goal was to replicate specific Photoshop behaviors that I missed in GIMP. While these scripts were developed with AI assistance, they have been manually tested and verified to work correctly in Linux Mint and Windows 10 environments.
